...
__global__ void TriLinearForward(const int nthreads, const float *lut, const float *image, float *output, const int dim, const int shift, const float binsize, const int width, const int height, const int batch)
{
    CUDA_1D_KERNEL_LOOP(index, nthreads)
    {
        ...
        int r_id = floor(r / binsize); // R0
        int g_id = floor(g / binsize); // G0
        int b_id = floor(b / binsize); // B0

        float r_d = fmod(r, binsize) / binsize; // delta_r
        float g_d = fmod(g, binsize) / binsize; // delta_g
        float b_d = fmod(b, binsize) / binsize; // delta_b
