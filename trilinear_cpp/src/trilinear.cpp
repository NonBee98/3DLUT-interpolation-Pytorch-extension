...
void TriLinearForwardCpu(const float *lut, const float *image, float *output, const int dim, const int shift, const float binsize, const int width, const int height, const int channels, const int batch)
{
    ...
    for (int batch_index = 0; batch_index < batch; ++batch_index)
    {
        ...
        for (int index = 0; index < output_size; ++index)
        {
            ...
            int id000 = b_id + g_id * dim + r_id * dim * dim;
            int id100 = b_id + 1 + g_id * dim + r_id * dim * dim;
            int id010 = b_id + (g_id + 1) * dim + r_id * dim * dim;
            int id110 = b_id + 1 + (g_id + 1) * dim + r_id * dim * dim;
            int id001 = b_id + g_id * dim + (r_id + 1) * dim * dim;
            int id101 = b_id + 1 + g_id * dim + (r_id + 1) * dim * dim;
            int id011 = b_id + (g_id + 1) * dim + (r_id + 1) * dim * dim;
            int id111 = b_id + 1 + (g_id + 1) * dim + (r_id + 1) * dim * dim;
            ...
            output[batch_start_index + index] =
                ...
                w011 * lut[id011] + w111 * lut[id111]; // compute R channel

            output[batch_start_index + index + output_size] =
                ...
                w011 * lut[id011 + shift] + w111 * lut[id111 + shift]; // compute G channel

            output[batch_start_index + index + output_size * 2] =
                ...
                w011 * lut[id011 + shift * 2] + w111 * lut[id111 + shift * 2]; // compute B channel
        }
    }
}
